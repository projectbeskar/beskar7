...
| `rbac.create` | `true` | Create RBAC resources for the manager. With `watchNamespaces` empty, renders a cluster-scoped ClusterRole + ClusterRoleBinding. With `watchNamespaces` set, renders a minimal ClusterRole + per-namespace Role/RoleBinding pairs (see `watchNamespaces`). | Also renders the metrics RBAC (`*-metrics-auth-role` / `-rolebinding`, which let the manager authenticate scrapers through the apiserver, and `*-metrics-reader` to bind to your scraper's identity).
