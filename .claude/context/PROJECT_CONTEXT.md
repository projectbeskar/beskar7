...
>
> **Last meaningful update:** 2026-05-02 — PR-5.2 lands inspection-handler bearer auth, body cap, TLS, and the ConfigMap+annotation handoff. SEC-1 fully closed; BUG-1 fully closed (D-005 invariant proven by `grep -n "client.Status().Update" controllers/inspection_handler.go` returning empty). SEC-3 closed as a free closure (RBAC markers no longer carry literal `// comment` verbs). Mint-on-inspection wiring lands in `triggerInspection` with per-host plaintext storage in a Secret (D-006). PR-5.3 still required for BLOCK-2 server side (bootstrap GET endpoint).
...
| ID | Area | Issue | Where |
|---|---|---|---|
| BLOCK-2 | CAPI conformance | **Partially closed by PR-1.1 + PR-5.1.** PR-1.1: controller reads bootstrap secret, signals URL via annotation, PhysicalHost persists to `Status.Bootstrap.URL`. PR-5.1: CRD now carries `TokenHash`/`IssuedAt`/`ExpiresAt` on `BootstrapStatus`, and `internal/auth` ships `MintToken`/`Verify`/`LifetimeFor`. Full closure still requires PR-5.2 (mint-on-inspection wiring + bearer auth on the inspection POST) and PR-5.3 (manager serves `GET /api/v1/bootstrap/{ns}/{name}` with the same auth). | `controllers/beskar7machine_controller.go`, `controllers/physicalhost_controller.go`, `internal/auth/` |
