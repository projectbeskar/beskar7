...
>
> **Last meaningful update:** 2026-05-02 — PR-5.1 lands the token primitive (`internal/auth.MintToken` / `Verify` / `LifetimeFor`) and adds `TokenHash`, `IssuedAt`, `ExpiresAt` to `PhysicalHostStatus.Bootstrap`. CRDs in `config/crd/bases/` and `charts/beskar7/crds/` regenerated and in sync. Mint-on-inspection wiring is intentionally deferred — the consumer (HTTP handlers) does not yet exist. BLOCK-2 remains "partially closed" until PR-5.3 ships the GET endpoint that exchanges the bearer token for bootstrap bytes.
