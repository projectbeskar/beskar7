...
>
> **Last meaningful update:** 2026-05-02 — PR-5.3 lands the bootstrap GET endpoint (`GET /api/v1/bootstrap/{ns}/{name}`) on the same `:8082` HTTPS surface as the inspection POST, gated by the same per-host bearer-token middleware (`auth.RequireBearer` + `newBearerTokenVerifier`). The `inspection_handler.go` setup helper is renamed `SetupInspectionServer` → `SetupCallbackServer` to reflect that the server now hosts two endpoints. `triggerInspection` gains a no-re-mint guard: if `Status.Bootstrap.TokenHash != "" && now < ExpiresAt`, the existing token is reused so in-flight bearer headers (already on the iPXE kernel cmdline of a booting host) stay valid across reconciles. **BLOCK-2 fully closed**.
