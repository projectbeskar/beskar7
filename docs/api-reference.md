...
All four Beskar7 CRDs carry the CAPI provider-contract labels and clusterctl's labels:
...
They are generated by `+kubebuilder:metadata:labels` markers on each Go type and shipped in both `config/crd/bases/*.yaml` and the chart-bundled CRDs (a `test/contract` test pins the labels and the byte identity of the two copies).
