...
| Field | Type | Description |
|---|---|---|
| `provisioned` | bool | True when the machine is fully provisioned: host claimed, inspected, `providerID` set. Source: `api/v1beta2/beskar7machine_types.go`. |
...
There is no `MachineProvisionedCondition` — the dead constant (declared but never set by any reconciler) has been removed from `api/v1beta2/beskar7machine_types.go`. `InfrastructureReady`, backed by `Status.Ready` and `Status.Initialization.Provisioned`, is the provisioned signal.
...
| Field | Type | Description |
|---|---|---|
| `provisioned` | bool | True when the infrastructure cluster is fully initialised: control-plane endpoint reachable and failure domains surfaced. Source: `api/v1beta2/beskar7cluster_types.go`. |
