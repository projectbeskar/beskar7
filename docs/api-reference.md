...
| `Beskar7ClusterTemplate` | `b7ct` | Namespaced | Template a `ClusterClass` references so Cluster API's topology controller can create a `Beskar7Cluster` per `Cluster`. |
...
`Beskar7ClusterTemplate` is a pure schema. A `ClusterClass` points `spec.infrastructure.templateRef` at it, and Cluster API's topology controller creates one `Beskar7Cluster` per `Cluster` from it. There is no controller and no webhook.
...
None. The CRD has no status subresource.
