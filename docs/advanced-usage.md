...
4. The reconciler mints a per-host bearer token (`internal/auth/token.go`) and stores the plaintext, its 60-minute expiry and the name of the claiming `Beskar7Machine` in a per-host Secret named `<host>-bootstrap-token` — the only credential the manager checks (D-029). The PhysicalHost reconciler mirrors the hash and lifetime into `Status.Bootstrap.{TokenHash,IssuedAt,ExpiresAt}` for operators.
...
6. Once hardware inspection passes, the **inspector itself** — not the target OS — fetches the bootstrap data: `GET https://<manager>:8082/api/v1/bootstrap/<ns>/<host>` with `Authorization: Bearer <plaintext>`. The manager validates the token against the host's Secret, and only while the host's `ConsumerRef` names the machine the token was minted for (`controllers/inspection_handler.go:newBearerTokenVerifier`), walks `PhysicalHost → ConsumerRef → Beskar7Machine → owner Machine → Spec.Bootstrap.DataSecretName → Secret`, and serves `secret.data["value"]` with `Cache-Control: no-store`. The `ConsumerRef` step is resolved only within the host's own namespace (`resolveConsumerBeskar7Machine`) — a `ConsumerRef` naming a `Beskar7Machine` in another namespace is treated as no consumer at all, so a host can never be pointed at another namespace's bootstrap data (SEC-12). The inspector writes those bytes byte-verbatim as `99_beskar7.yaml` (mode `0600`, root-owned) on the target image's `COS_OEM` partition, then reboots the host via host firmware into the provisioned OS, which applies the config on first boot (no `kexec`).
