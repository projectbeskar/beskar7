...
| `host ... has no valid Beskar7Machine consumer` | The host is not claimed (or its `ConsumerRef` names another namespace). Callbacks for an unclaimed host are always rejected (D-029). | Expected after the Beskar7Machine was deleted. Otherwise check the host's `spec.consumerRef`. |
| `get bootstrap-token Secret ...` | The per-host Secret `<host>-bootstrap-token` is missing or unreadable. | The Beskar7Machine reconciler has not yet run `triggerInspection` for this host, or the Secret was deleted. Check Beskar7Machine status. |
| `bootstrap-token Secret ... is not bound to the host's current consumer` | The token was minted for a different `Beskar7Machine` than the one the host's `ConsumerRef` names now — a re-claim that has not minted yet, a `ConsumerRef` edited by hand, or a Secret written by a release before D-029 on a host that was already `Ready` at the upgrade. | A new claim mints fresh credentials in its `triggerInspection`; the host must re-PXE to pick them up. Pre-upgrade tokens on `Ready` hosts are not needed: no callback follows `Ready`. |
| `no bootstrap token issued for host ...` | The Secret holds no `plaintext-token`. | As for a missing Secret. |
| `bootstrap token for host ... has no valid expiry` | The Secret's `token-expires-at` is missing or unparseable, so the token fails closed. | Do not hand-edit the Secret; let the reconciler mint (re-claim the host, or delete the Secret while the host is `InUse`). |
...
| `bootstrap token mismatch for host ...` | The plaintext on the wire does not hash to the token in the Secret. | Likely a stale iPXE cmdline. Rebuild the cmdline from the current Secret (`kubectl get secret <host>-bootstrap-token -o jsonpath='{.data.plaintext-token}' \| base64 -d`) and re-PXE the host. Persistent mismatch suggests cmdline truncation or shell-escape damage in your iPXE template. The Secret is the only credential checked; `status.bootstrap.tokenHash` only mirrors it. |
...
```bash
kubectl logs -n capb7-system deployment/capb7-controller-manager | grep -i forbidden
```

Identify the verb + resource being denied. Compare with the deployed ClusterRole:

```bash
kubectl auth can-i --list --as=system:serviceaccount:capb7-system:capb7-manager
```
...
kubectl logs -n capb7-system deployment/capb7-controller-manager --tail=500 > controller.log
