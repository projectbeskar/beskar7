...
| `infrastructure.cluster.x-k8s.io/boot-nonce` | `Beskar7Machine` controller | Persist hash + expiry to `Status.Bootstrap.{BootNonceHash,BootNonceExpiresAt}`, which `/boot` verifies. The consume record is the `/boot` handler's. |
