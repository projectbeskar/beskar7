...
| `Inspecting` / `Deploying` / `Ready` (claimed) | unchanged | BMC unreachable. The host keeps its state — the inspector and the installed OS do not need the BMC — and `RedfishConnectionReady=False (BMCUnreachable)` reports the outage until the BMC answers. Provisioning goes on meanwhile: the host still acts on the annotations in the table below, so it moves through its run as it would with the BMC answering. | `physicalhost_controller.go:retryTransientRedfishFailure`, `physicalhost_controller.go:applyRunAnnotations` |
...
Annotations consumed by the `PhysicalHost` reconciler. None of them needs the BMC: the reconciler acts on them when the BMC answers and when it cannot be reached at the network level. A BMC failure that needs a fix (credentials, certificate, TLS config, no `ComputerSystem`) writes its own `Error` over the host's state, so while one holds the connection the reconciler leaves them in place and acts on them in the pass that connects — except the inspector's two reports, which it applies before it tries the BMC in every pass:
