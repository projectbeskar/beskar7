...
```
   ...
   ³ Applied before the host tries its BMC as well, and kept the same way on a
     host still Inspecting. A /provision-failed report waiting with it wins.
```
...
| `InUse` | `Inspecting` | Beskar7Machine writes `inspection-request: inspect` annotation — also while the host's BMC is unreachable, if the machine powered the host on into the inspector just before the outage. | `beskar7machine_controller.go:triggerInspection` |
...
| `Deploying` | `Ready` | Inspector POSTs `POST /api/v1/provisioned/{ns}/{host}` (§4.4 of `docs/inspector-contract.md`). The provisioned handler patches `ProvisionedRequestAnnotation`; the PhysicalHost reconciler transitions to `Ready`, and clears the annotation on a later pass, once status shows `Ready` — the reconciler's patch writes metadata before status, so clearing it in the same pass would lose the report to a status write that fails. It applies the report before it tries the BMC, so a deployment that finishes during an outage provisions the machine rather than timing it out. A `/provision-failed` report waiting on the same host is applied instead, the conservative choice when both are present (one inspector run never sends both). | `controllers/provisioned_handler.go`, `physicalhost_controller.go:applyProvisionedRequestAnnotation` |
| `Inspecting` (claimed) | unchanged, then `Deploying` and `Ready` | `/provisioned` arrives before the host is `Deploying`: the inspector does not wait for the host (§9.2), and the host goes to `Deploying` only once the machine has validated the inspection report — which can come after the whole deployment when the controllers were away while a callback-only instance kept answering the inspector. The host keeps the annotation, and applies it in the pass after `inspect-complete` has moved it to `Deploying`. It clears the annotation without a transition if the host goes anywhere else — the machine rejects the hardware (`HardwareRequirementsNotMet`), the host is released — or if the report came before the run's inspection report. | `physicalhost_controller.go:applyProvisionedRequestAnnotation` |
...
| `Error` about the BMC, during deployment (claimed) | `Error` of the failed run | `/provision-failed` arrives for a host that v0.8.0 or earlier put in `Error` over `Deploying` for a BMC failure that needs a fix (credentials, certificate, TLS config, no `ComputerSystem`); the current controller keeps such a host `Deploying` (rows below), so only a host upgraded in that state reaches this row. The inspector does not need the BMC, so the report still applies; the host takes the inspector's reason and keeps it until it is released. | `physicalhost_controller.go:deployInterruptedByBMCError` |
...
| any | `Error` | Credentials missing, credentials the Secret does not authorise for the address (`CredentialsNotAuthorized`), TLS-config conflict, Redfish connection or query failed, or an unreachable BMC — on a host that is unclaimed or `InUse`. A claimed host that is `Inspecting`, `Deploying` or `Ready`, or in the `Error` of a failed run, keeps its state (rows below). | `physicalhost_controller.go:reconcileNormal`, `bmc_access.go:resolveBMCAccess` |
| `Inspecting` / `Deploying` / `Ready` (claimed) | unchanged | BMC unreachable, or another Redfish/BMC-level fault that needs a fix — missing or refused credentials, credentials the Secret does not authorise for the address, a certificate the client rejects, a malformed address, a BMC with no `ComputerSystem`, the `insecureSkipVerify`/CA-bundle conflict. The host keeps its state — the inspector and the installed OS do not need the BMC — and `RedfishConnectionReady=False` reports the fault (`BMCUnreachable`, `CredentialsNotAuthorized`, `MissingCredentials`, `InsecureCABundleConflict`, `CABundleFetchFailed`, `RedfishConnectionFailed`, or `RedfishQueryFailed`) until it clears. Provisioning goes on meanwhile: the host still acts on the annotations in the table below, so it moves through its run as it would with the BMC answering. | `physicalhost_controller.go:retryTransientRedfishFailure`, `physicalhost_controller.go:setConnectionError`, `physicalhost_controller.go:applyRunAnnotations` |
| `Ready` (claimed) | unchanged | An `inspection-request` annotation is consumed without effect. The `Beskar7Machine` can send `inspect-complete` a second time after reading a copy of the host from before its first request was applied, and a host that kept a `/provisioned` report while `Inspecting` is `Ready` one pass after it is `Deploying`. | `physicalhost_controller.go:applyInspectionRequest` |
...
| `Error` | `Available`, or `InUse` if claimed | The underlying error clears (BMC reachable again, secret fixed or annotated, TLS config fixed). Not the `Error` of a failed run, which a claimed host keeps until it is released. | `physicalhost_controller.go:reconcileNormal` |
...
Annotations consumed by the `PhysicalHost` reconciler. None of them needs the BMC, so the reconciler acts on them in every pass: when the BMC answers, when it cannot be reached at the network level, and when it fails in a way that needs a fix (credentials, certificate, TLS config, no `ComputerSystem`). The inspector's two reports are applied first, before the BMC is tried:
...
| `infrastructure.cluster.x-k8s.io/bootstrap-token`, `infrastructure.cluster.x-k8s.io/boot-nonce` | Nothing (retired, D-029) | Removed on sight, never read. The callback credentials live only in the `<host>-bootstrap-token` Secret; the reconciler mirrors its hashes and expiries into `Status.Bootstrap.{TokenHash,IssuedAt,ExpiresAt,BootNonceHash,BootNonceExpiresAt}` for operators, and follows it through a Secret watch. The consume record is the `/boot` handler's. |
...
| `infrastructure.cluster.x-k8s.io/provisioned-request` | `ProvisionedHandler` (HTTP) | Transition `Status.State` from `Deploying` to `Ready` (D-015). Value: `"provisioned"`. Applied before the BMC is tried, and after `provision-failed-request`, which wins when both are present. On a claimed host still `Inspecting`, left in place until the host is `Deploying`. Cleared on the pass after the action, once status shows `Ready`, or without one when the host cannot be provisioned by it. |
...
Look at the `RedfishConnectionReady` condition reason — it is one of `BMCUnreachable`, `CredentialsNotAuthorized`, `MissingCredentials`, `InsecureCABundleConflict`, `CABundleFetchFailed`, `RedfishConnectionFailed`, or `RedfishQueryFailed`. Fix the credentials Secret, its annotations or the BMC address; the next reconcile transitions to `Available`.

Three retry cadences sit behind that. A host whose BMC is simply unreachable — reason `BMCUnreachable`, message `BMC unreachable (connection refused)` or similar, covering a refused or reset connection, no route, a DNS failure, a timeout, or a 502/503/504 from a BMC that is still booting — is retried every 15 seconds, flat, and enrols on the first attempt after the BMC answers. Nothing has to be fixed for that to clear, and a `Beskar7Machine` holding the host waits for it instead of failing (see [Beskar7Machine → A BMC outage is not a terminal failure](beskar7machine.md#a-bmc-outage-is-not-a-terminal-failure)). A host refused with `CredentialsNotAuthorized` (its credentials Secret does not [authorise the address](physicalhost.md#binding-the-credentials-to-their-bmc), so nothing was sent) or `InsecureCABundleConflict` is re-checked every 5 minutes, since only an edit clears it; a `Beskar7Machine` holding a host refused with `CredentialsNotAuthorized` waits for that edit instead of failing. Every other Redfish failure needs a change to the spec, the Secret or the BMC itself, so it backs off exponentially (5s, 10s, 20s, … capped at 30 minutes) and a host that has been failing for a while can take a few minutes to notice the fix. Editing the `PhysicalHost` or its credentials Secret, annotations included, wakes the controller immediately.
...
- `failed to retrieve credentials: …`: the named Secret does not exist, cannot be read, or is missing `username`/`password`. Create or fix it.
- `BMC credentials not sent: …`: the credentials Secret does not authorise this host's address (`RedfishConnectionReady=False (CredentialsNotAuthorized)`), so nothing was sent to the BMC. The message names what is missing. Check that the address really is this host's BMC, then add it to the Secret's `beskar7.infrastructure.cluster.x-k8s.io/bmc-addresses` annotation (and set `bmc-insecure-transport: "true"` for an `http://` address or `insecureSkipVerify: true`). A `Beskar7Machine` holding the host reports `InfrastructureReady=False (WaitingForBMC)` meanwhile and carries on once the Secret is annotated. See [Troubleshooting → 17](troubleshooting.md#17-physicalhost-reports-credentialsnotauthorized).
