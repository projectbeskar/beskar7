...
| any | `Error` | Credentials missing, TLS-config conflict, Redfish connection or query failed, or an unreachable BMC — on a host that is unclaimed or `InUse`. A claimed host that is `Inspecting`, `Deploying` or `Ready`, or in the `Error` of a failed run, keeps its state (rows below). | `physicalhost_controller.go:reconcileNormal` |
| `Inspecting` / `Deploying` / `Ready` (claimed) | unchanged | BMC unreachable, or another Redfish/BMC-level fault that needs a fix — missing or refused credentials, a certificate the client rejects, a malformed address, a BMC with no `ComputerSystem`, the `insecureSkipVerify`/CA-bundle conflict. The host keeps its state — the inspector and the installed OS do not need the BMC — and `RedfishConnectionReady=False` reports the fault (`BMCUnreachable`, `MissingCredentials`, `InsecureCABundleConflict`, `CABundleFetchFailed`, `RedfishConnectionFailed`, or `RedfishQueryFailed`) until it clears. Provisioning goes on meanwhile: the host still acts on the annotations in the table below, so it moves through its run as it would with the BMC answering. | `physicalhost_controller.go:retryTransientRedfishFailure`, `physicalhost_controller.go:setConnectionError`, `physicalhost_controller.go:applyRunAnnotations` |
