...
2. **Find or claim a host.** `findAndClaimOrGetAssociatedHost` runs three lookups in order:
    ...
    2. List PhysicalHosts in the namespace and return the one whose `Spec.ConsumerRef.Name` matches this Beskar7Machine — covers the window between claim and `ProviderID` assignment. Without this branch the controller would forget its own claim after the first reconcile (the host has transitioned to `InUse` so the next branch's `Available` filter skips it). The match also requires `ConsumerRef.Namespace` to be empty or equal to the host's own namespace (`resolveConsumerBeskar7Machine`), so a host claimed "by" a same-named machine in a different namespace is never mistaken for this machine's host (SEC-12).
...
5. **Trigger inspection.** When the host transitions to `InUse`:
    ...
    - Mint the per-host bearer token and boot nonce into the host's Secret (`<host>-bootstrap-token`), with their expiries and the name of this `Beskar7Machine` under `consumer`, in a single write (decision D-029). A credential is reused only when the Secret is already bound to this machine and holds it unexpired (the nonce also unconsumed); re-using avoids invalidating in-flight kernel cmdlines. A Secret bound to the host's previous machine always gets fresh credentials, so a token from one claim never opens the next. The write carries the Secret's `resourceVersion`, so a decision made from a stale cache fails with a conflict and is retried instead of replacing a newer mint. Nothing is written to the PhysicalHost for the credentials: the host reconciler mirrors the Secret into `Status.Bootstrap`, and the callback server reads the Secret itself. The plaintext is never logged. See `internal/auth/token.go` and decision D-029.
    - Patch the `inspection-request` annotation on the host with value `inspect`. The host reconciler transitions to `Inspecting`.

    If the host is in `Error` instead because it cannot reach its BMC, or because its credentials Secret does not authorise the BMC's address, the machine waits rather than failing and picks up here once the host is back at `InUse` — see [A BMC outage is not a terminal failure](#a-bmc-outage-is-not-a-terminal-failure).
...
| `PhysicalHostError` | The claimed `PhysicalHost` entered `StateError` for a Redfish/BMC-level reason that needs a change to clear — missing or refused credentials, a certificate the client rejects, a malformed address, a BMC with no `ComputerSystem`, the `insecureSkipVerify`/CA-bundle conflict — rather than a reported deploy failure. Only before inspection, while the host is still `InUse`: the same fault on a host already `Inspecting`, `Deploying` or `Ready` leaves both the host's state and the machine alone. An unreachable BMC is not one of these, and neither is a credentials Secret that does not authorise the BMC's address; see below. |
...
- **Before inspection** (the host was `InUse`): the host goes to `Error`, and the machine waits with `InfrastructureReady=False`, reason `WaitingForBMC`, a message quoting the host's error, and its phase unchanged. It is re-checked every 30 seconds and at once whenever the host changes, and once the host is back at `InUse` it triggers inspection as if nothing had happened, with the reason back at `PhysicalHostNotReady`. If the machine had already powered the host on into the inspector when the outage began, the host goes to `Inspecting` instead and publishes the bearer token and boot nonce the inspector needs, and the machine follows it as below.
- **Inspecting, Deploying or Ready**: the host keeps its state — the inspector and the installed OS do not need the BMC — and only its condition reports the outage. Provisioning goes on without the BMC: the host reads the inspection report, applies the machine's `inspect-complete` and the inspector's `/provisioned` or `/provision-failed` report, so a host that is inspected and deployed during the outage provisions its machine during it, and a deployment reported failed fails the machine with `DeploymentFailed` at once. A machine provisioned this way skips its best-effort clear of the one-time boot override, which needs the BMC. A provisioned machine stays `Ready`. The inspection and deployment timeouts keep running meanwhile, and fire only if the inspector itself stops reporting.
- **After the run failed** (the inspector's `/provision-failed` report, or an inspection timeout): the host keeps the run's `Error` and message through the outage, so the machine fails with `DeploymentFailed` or `InspectionTimedOut` rather than waiting.

Credentials the host may not use are handled the same way. When its credentials Secret does not authorise the BMC's address — no `beskar7.infrastructure.cluster.x-k8s.io/bmc-addresses` annotation, an annotation that does not list the address, or an `http://` or `insecureSkipVerify: true` connection without `bmc-insecure-transport: "true"` — the host sends nothing to the BMC and reports `RedfishConnectionReady=False` with reason `CredentialsNotAuthorized`. The machine waits with `WaitingForBMC` before inspection and carries on from `Inspecting`, `Deploying` or `Ready`, exactly as above. Unlike an outage, this does not clear by itself: annotating the Secret recovers the host at once, through its Secret watch, and nothing is reprovisioned. See [PhysicalHost → Binding the credentials to their BMC](physicalhost.md#binding-the-credentials-to-their-bmc).
...
- **A machine that fails after its Node joined** is left to the `InfrastructureReady` check, 90 minutes later; a failed machine never recovers, so the check replaces it even if its Node is still serving. A BMC fault does not fail it: a provisioned host whose BMC credentials, certificate or connection break stays `Ready`, and only its `RedfishConnectionReady` condition reports the fault (see [Terminal failures](#terminal-failures)). Leave the check out, or annotate the `Machine` with `cluster.x-k8s.io/skip-remediation`, if you would rather make that call by hand.
...
1. Locate the claimed host. **`ConsumerRef` ownership is the source of truth**, not `ProviderID`: a machine deleted before inspection completes holds a claimed host but has no `ProviderID` yet, so a ProviderID-only lookup would miss it and strand the host (#107). `ProviderID`, when set and parseable, is used as a fast-path `Get` to avoid a list in the common provisioned case; when it is unset, malformed, or points at a host this machine does not own, the controller scans the namespace for a `PhysicalHost` whose `Spec.ConsumerRef.Name` matches and whose `ConsumerRef.Namespace` is empty or equal to that host's own namespace (SEC-12 — a same-named machine claiming a host in a different namespace does not match). Once found:
    - Best-effort: open the Redfish client and call `ClearBootSourceOverride` then `SetPowerState(Off)` (graceful). All errors are logged and swallowed so a dead BMC cannot strand the finalizer. A credentials Secret that does not authorise the host's address skips both calls the same way; nothing is sent to the BMC.
