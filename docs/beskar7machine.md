...
- **Inspecting, Deploying or Ready**: the host keeps its state — the inspector and the installed OS do not need the BMC — and only its condition reports the outage, so the machine carries on untouched. A provisioned machine stays `Ready`. The inspection and deployment timeouts keep running meanwhile, and the host reads the inspection report and applies the machine's `inspect-complete` only once it can reach the BMC again, so an outage that starts before the host has read the report and outlasts the remaining inspection timeout still ends in `InspectionTimedOut`. The inspector's `/provision-failed` and `/provisioned` reports are the exception: the host applies them before it tries the BMC, so a deployment reported failed during the outage fails the machine with `DeploymentFailed` at once, and one reported finished provisions it (the machine then skips its best-effort clear of the one-time boot override, which needs the BMC). A `/provisioned` report that arrives while the host is still `Inspecting`, waiting for its BMC to apply `inspect-complete`, is kept and applied once the host is `Deploying`, so the machine is provisioned when the BMC answers instead of waiting out `--deployment-timeout`.
