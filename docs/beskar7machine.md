...
| `InfrastructureReady` | True once the host reaches `Ready` (the inspector's provisioned callback was received) and `ProviderID` is set. | `Provisioned` | `PhysicalHostNotReady` (host claimed, still deploying); `WaitingForBMC` (the host cannot reach its BMC — not terminal, see [below](#a-bmc-outage-is-not-a-terminal-failure)); terminal — see [Terminal failures](#terminal-failures). |
...
`WaitingForBMC` is not terminal, but it is still `InfrastructureReady=False`, and so is the `Ready` summary Cluster API mirrors onto the owning `Machine`. A `MachineHealthCheck` sees only that status and how long it has held — not the reason — so whether it waits the outage out depends on the `timeoutSeconds` of its `InfrastructureReady` check: with `0` it remediates the waiting machine at once.
