...
To install a build of the current tree instead of a release, publish it into a clusterctl local repository first: `make clusterctl-override VERSION=v0.6.1` writes `~/.cluster-api/overrides/infrastructure-beskar7/v0.6.1/`, and `clusterctl init --infrastructure beskar7:v0.6.1` uses it without touching the network. CI does exactly that on every pull request and runs the smoke suite against the result.
