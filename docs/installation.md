...
To install a build of the current tree instead of a release, publish it into a clusterctl local repository first: `make clusterctl-override VERSION=v0.8.0` writes `~/.cluster-api/overrides/infrastructure-beskar7/v0.8.0/`, and `clusterctl init --infrastructure beskar7:v0.8.0` uses it without touching the network. CI does exactly that on every pull request and runs the smoke suite against the result.
...
`clusterctl move` builds its object graph only from CRDs that carry the `clusterctl.cluster.x-k8s.io` label. `clusterctl init` adds that label to everything it installs, but neither the Helm chart nor the release manifest goes through `clusterctl init`, so the five beskar7 CRDs carry it themselves, together with `cluster.x-k8s.io/provider: infrastructure-beskar7` (the component label the CAPI provider contract asks for; every other beskar7 component carries the same two). `Beskar7Cluster`, `Beskar7Machine` and `Beskar7MachineTemplate` objects are then moved through their owner references to the `Cluster`, as with any provider. `PhysicalHost` also carries `clusterctl.cluster.x-k8s.io/move-hierarchy`: nothing owns a host, so without it a move would discover the hosts and leave every one of them behind. With it, every `PhysicalHost` in the namespace being moved goes along, together with the bootstrap-token Secret and inspection-result ConfigMap it owns, and is deleted from the source afterwards like any other namespaced object.
...
`PhysicalHost` has no owning `Cluster` and does not consult `spec.paused`; it only understands its own `cluster.x-k8s.io/paused` annotation, and it keeps reconciling throughout a move. This is deliberate. `clusterctl move` only `Create`s objects on the target — it never carries `.status` across — so a moved `PhysicalHost` always lands with `status.state` empty, even one that was fully provisioned. A `PhysicalHost` reconciler that stayed quiet until the target `Cluster` unpaused would leave that empty state in place, and the still-paused `Beskar7Machine` that already provisioned the host — its `spec.providerID` still names it; `spec` is never dropped by a move — would find an apparently-unclaimed host the moment it woke up and re-run inspection on hardware that is already serving. Instead, `PhysicalHost` recognises a claimed host whose consumer's `spec.providerID` already names it and restores `status.state: Ready` from that fact alone, before it ever calls the BMC; `Beskar7Machine`, in turn, never boots the inspector while it already holds a host by `providerID` — it waits for the host to catch up instead. Both sides of the pair are therefore back exactly where they were by the time the target `Cluster` unpauses, with no operator step required.

**What a move keeps, and what it does not.** Only `spec` and `metadata` cross a move; `status` never does. For a `PhysicalHost` this means `status.state`, `status.ready` and `status.errorMessage` are rebuilt automatically as above, but `status.inspectionReport` and the run timestamps (`status.inspectionTimestamp`, `status.deployingTimestamp`) are not — they describe the run that produced the current claim, not the claim itself, and nothing in `spec` can reconstruct them. Capture the inspection report first if you need it for a record: `kubectl get physicalhost <name> -o jsonpath='{.status.inspectionReport}'`. Separately, `clusterctl move` refuses to start while any `Machine` lacks a `status.nodeRef`, so only a pair that has already reached `Ready` / `Provisioned` can be moved at all — a host or machine still mid-run is excluded by clusterctl's own precondition, not by anything beskar7 does.

The source side of a move deletes the source `Beskar7Machine`. An ordinary deletion powers the host off and releases its claim, which would fight the pair now live on the target — so `Beskar7MachineReconciler` checks for the `clusterctl.cluster.x-k8s.io/delete-for-move` annotation the mover sets immediately before deleting, and skips both the Redfish power-off and the claim release when it is present; the hardware keeps serving under the pair on the target.
...
Moving a Ready / Provisioned pair is exercised against envtest, replaying the mover's own object graph and ordering, in `controllers/clusterctl_move_test.go`. Moving a pair still mid-run is not possible — clusterctl's own `nodeRef` precondition excludes it — so there is nothing further to rehearse there; still, rehearse a real move on a lab cluster before relying on it for a live workload cluster.
