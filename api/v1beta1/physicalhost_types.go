...
type RedfishConnection struct {
	...
	// CABundleSecretRef is the name of a Secret in the same namespace as the
	...
	//
	// When unset (empty string), behaviour is unchanged: the manager builds an
	// http.Client whose TLS Transport uses system roots and the
...
func (in *PhysicalHostSpec) DeepCopyInto(out *PhysicalHostSpec) {
	*out = *in
	// RedfishConnection contains a pointer field (InsecureSkipVerify) — must
	// use the type's own DeepCopyInto so the pointer is not shared.
