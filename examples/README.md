...
| `clusterclass.yaml` | ClusterClass | The `Beskar7ClusterTemplate` a `ClusterClass` points at, the machine templates its control-plane and worker classes use, and a topology `Cluster` that consumes the class. The control-plane/bootstrap halves use Kubeadm as the portable illustration — substitute your own providers. See [Beskar7ClusterTemplate](../docs/beskar7clustertemplate.md). | `kubectl apply -f clusterclass.yaml` |
| `host-pools.yaml` | Host selection | Two labelled PhysicalHosts plus a control-plane and a worker `Beskar7MachineTemplate`, each with a `hostSelector`, so the two pools claim disjoint inventory instead of the first `Available` host. Merge with a full cluster fixture. | `kubectl apply -f host-pools.yaml` |
