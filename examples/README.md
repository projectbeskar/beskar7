...
| `kairos-k0s-start-gate.yaml` | Image-side stage (k0s) | **Required for k0s; verified with the stage above.** Bake into `COS_OEM` as `/oem/05_beskar7_k0s_gate.yaml`. Keeps `k0scontroller`/`k0sworker` from starting on the recovery-partition install boot (where a joiner otherwise becomes a voting etcd member and is then rebooted, killing quorum) or before the bootstrap provider has written its arguments. Opens on `/etc/k0s/.capi-args-ready`, written by cluster-api-provider-kairos from [kairos-io/cluster-api-provider-kairos#99](https://github.com/kairos-io/cluster-api-provider-kairos/pull/99) — with an older provider k0s never starts. | bake into the image, not `kubectl apply` |
